...
That last one surprises people. Installed packages are near-always shared state
— a symlinked virtualenv, a hoisted `node_modules`, a vendor directory — so one
...
never been fixed, with 1,116 real tests sitting unreachable from the build graph
— and a second defect underneath it where the runner stripped environment
...
- **Scope.** State the window measured and the window claimed. Absolutes —
  *never, always, zero, none, every* — require a population-wide command.
